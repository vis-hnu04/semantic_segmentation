...
class VGGNet(VGG):
    def __init__(self, pretrained=True, model='vgg16', requires_grad=True, remove_fc=True, show_params=False):
      ...
      #  checkpoint = torch.load("/home/uic55883/.cache/torch/checkpoints/vgg16-397923af.pth")
        if pretrained:
    ...
    for v in cfg:
        if v == 'M':
            ...
            if batch_norm:
                ...
                #if dropout < 2:
                 #   layers += [conv2d, nn.ReLU(inplace=True), nn.Dropout(p=0.2)]
                  #  dropout += 1
                #else:
...
if __name__ == "__main__":
    batch_size, n_class, h, w = 1, 2,640, 1024


    # test output size
    vgg_model = VGGNet(requires_grad=True)
   # print (vgg_model.classifier)
    input = torch.autograd.Variable(torch.randn(batch_size, 3, 640, 1024))
    output = vgg_model(input)
    print(output['x5'].size())
